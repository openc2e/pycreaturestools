...
class cursor:
    ...
    def read_unknown_s32le(self, expected):
        ...
        if unknown not in expected:
            ...
            raise Exception(
                "Expected {}; but got {} ({:#x}) - next few bytes: {}".format(
                    expected_string, unknown, unknown, self.read(15)
                )
            )
...
def find_pray_block(c):
    ...
    while c.peek():
        ...
        sys.stderr.write(
            f"{block_type}, {block_name}, {length_compressed}, {length_decompressed}, {flags}\n"
        )
...
def binrepr(buf):
    ...
    for c in buf:
        if (
            c
            in b"0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~ "
        ):
            out += chr(c)
        else:
            out += "\\x" + bytes([c]).hex()
...
class printerhelper:
    ...
    def unknown_float(self, buf):
        ...
        print(
            '<div class="unknown" {}>'.format(
                'style="display: inline-block"' if self.inline else ""
            )
        )
    ...
    def string(self, s):
        print(
            '<div class="string"{}><span class="raw">{}</span>{}</div>'.format(
                'style="display: inline-block"' if self.inline else "",
                binrepr(struct.pack("<I", len(s))),
                html.escape(s),
            )
        )
...
def parse_limb():
    ...
    printer.named(
        "limb attachment data? - 16 rows x 4 columns = 64 bytes", data[i : i + 64]
    )
    i += 64
...
def start_parse_object(type):
    ...
    print('<div class="object">')
    print(
        '<div class="object-name">{} index={} type={}</div>'.format(
            types[object_type], object_index, object_type
        )
    )
...
def parse_body():
    ...
    printer.named(
        "body attachment data - 16 rows x 12 columns = 192 bytes", data[i : i + 192]
    )
    i += 192
...
def parse_creaturehead():
    ...
    printer.named(
        "head attachment data - 16 rows x 4 columns = 64 bytes (Creatures Village has extra head data though...)",
        data[i : i + 64],
    )
    i += 64
...
def parse_linguisticfaculty():
    ...
    for category in (
        ...
        for _ in range(num_words):
            printer.set_inline(True)
            length = struct.unpack("<I", data[i : i + 4])[0]
            script = data[i + 4 : i + 4 + length].decode(
                "cp1252"
            )  # first is an override? e.g. daphne - me
