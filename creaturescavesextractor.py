        ...
        with tempfile.TemporaryDirectory() as tempdir:
            with open(filename, "rb") as f:
                ...
                if (
                    ...
                    or magic.startswith(b"PK00PK\x03\x04")
                    or magic.startswith(b"Rar!")
                    or magic.startswith(b"7z\xbc\xaf")
                ):
                    # archives
                    try:
                        ...
                        sys.stderr.write(
                            f"Error extracting {filename} - try an external program\n"
                        )
                        ...
                        if magic.startswith(b"Rar!"):
                            shutil.copy(
                                filename,
                                os.path.join(tempdir, base_output_name + ".rar"),
                            )
                            os.rename(tempdir, output_dir)
                    ...
                    if glst_name.endswith(".DSEX.glist.creature"):
                        filename = re.sub(
                            r"\.DSEX\.glist\.creature$", ".ds.creature", glst_name
                        )
