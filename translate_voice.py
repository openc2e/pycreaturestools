...
class VoiceProcessor:
    ...
    def transform(self, chars):
        ...
        data = VoiceLookup[chars[0]] & (
            VoiceLookup[chars[1] + 27] & VoiceLookup[chars[2] + 27 + 27]
        )
        if chars[1] == 26:
    ...
    def getsyllable(self):
        if self.offset + 1 >= len(self.speech):
            return None
        chars = [
            self.speech[self.offset - 1],
            self.speech[self.offset],
            self.speech[self.offset + 1],
        ]
...
while True:
    ...
    os.popen2(
        "mplayer -ao alsa ~/creatures2/Sounds/"
        + x[0].lower()
