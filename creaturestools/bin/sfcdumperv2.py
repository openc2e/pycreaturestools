...
class PointerTool(SimpleObject):
    def read(self, ar):
        ...
        self.relx = ar.read_u32le()
        self.rely = ar.read_u32le()
        self.bubble = ar.read_object("Bubble")
        self.text = ar.read(25)
        self.text = self.text[: self.text.find(b"\0")].decode("ascii")
...
def read_sfc_file(ar):
    ...
    for _ in range(num_deathrow):
        deathrow_obj = ar.read_object("Object")

    num_events = ar.read_u32le()
    for _ in range(num_events):
        event_obj = ar.read_object("Object")

    current_score = ar.read_u32le()
    current_health = ar.read_u32le()

    print("current_score {}".format(current_score))
    print("current_health {}".format(current_health))

    hatchery_eggs = ar.read_u32le()
    natural_eggs = ar.read_u32le()
    dead_norns = ar.read_u32le()
    live_norns = ar.read_u32le()
    breeders_score = ar.read_u32le()

    print(f"{hatchery_eggs=}")
    print(f"{natural_eggs=}")
    print(f"{dead_norns=}")
    print(f"{live_norns=}")
    print(f"{breeders_score=}")

    systick = ar.read_u32le()
