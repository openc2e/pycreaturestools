...
from creaturestools.creaturesarchive import *
...
def main():
    ...
    data = decompress_creaturesarchive_compressed_file(input_filename)
    cur = cursor(data)
        ...
        if unknown not in expected:
            ...
            raise Exception(
                "Expected {}; but got {} ({:#x}) - next few bytes: {}".format(
                    expected_string, unknown, unknown, cur_.read(15)
                )
            )
    ...
    def read_command_list(command_group_id):
        ...
        for i in range(number):
            ...
            if command_signature == b"*":
                ...
                if command_signature.startswith(
                    b"-"
                ):  # seems to mean special handling/control flow?
                    needed_arguments -= 1
