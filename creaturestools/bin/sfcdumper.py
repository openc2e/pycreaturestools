...
class MapData:
    def read(self, f):
        ...
        if version == 1:
            ...
            assert x == 0 or x == 4, "fourth bytes of MapData were not zero but " + str(
                x
            )
            x = read32(f)  # TODO
...
class Object:
    ...
    def partialread(self, f):
        ...
        if version == 1:
            ...
            if x != 0xFFFFFFFF:
                print(
                    "* mysterious physicsish value (probably GRAV related): " + str(x)
                )
            self.accg = read32(f)
            self.velx = reads32(f)
            self.vely = reads32(f)
            ...
            self.flags = read8(f)
            assert self.flags == 0 or self.flags == 1 or self.flags == 3, str(
                self.flags
            )
        ...
        for i in range(numscripts):
            ...
            script = readstring(f)
            print(
                "event #{eventno} for {family}, {genus}, {species}: scrp {family} {genus} {species} {eventno},{script}".format(
                    eventno=eventno,
                    family=family,
                    genus=genus,
                    species=species,
...
class SimpleObject(Object):
    def read(self, f):
        ...
        print(
            "read "
            + str(len(self.pickup_handles))
            + " pickup handles and "
            + str(len(self.pickup_points))
            + " pickup points"
        )
...
class Blackboard(CompoundObject):
    def read(self, f):
        ...
        print(
            "colours: "
            + str(self.backgroundcolour)
            + ", "
            + str(self.chalkcolour)
            + ", "
            + str(self.aliascolour)
        )
...
class Lobe:
    def read(self, f):
        ...
        print(
            "lobe has nominal threshold "
            + str(self.nominalthreshold)
            + ", leakage rate "
            + str(self.leakagerate)
            + ", rest state "
            + str(self.reststate)
            + " and input gain "
            + str(self.inputgain)
...
class DendriteDetails:
    def read(self, f):
        ...
        print(
            "src lobe %d, min %d, max %d, spread %d, fanout %d, minltw %d, maxltw %d, minstr %d, maxstr %d"
            % (
                self.sourcelobe,
                self.minimum,
                self.maximum,
        ...
        print(
            "migration rule %d, relax suscept %d, relax STW %d, LTW gain rate %d"
            % (self.migrationrule, self.relaxsuscept, self.relaxstw, self.ltwgainrate)
        )
...
for i in range(numscripts):
    ...
    script = readstring(f)
    print(
        "event #{eventno} for {family}, {genus}, {species} (global): scrp {family} {genus} {species} {eventno},{script}".format(
            eventno=eventno, family=family, genus=genus, species=species, script=script
        )
    )
...
for i in data.rooms:
    ...
    for j in range(4):
        ...
        for k in i.doordirections[j]:
            if k != i.doordirections[j][0]:
                print(
                    ",",
                )
            print(
                "openness " + str(k.openness) + " to room #" + str(k.otherroom),
            )
        print()
    ...
    print(
        "inorganic nutrients: "
        + str(i.inorganicnutrients)
        + ", organic nutrients: "
        + str(i.organicnutrients)
    )
    print("temperature: " + str(i.temperature) + ", heat source: " + str(i.heatsource))
    print(
        "pressure: " + str(i.pressure) + ", pressure source: " + str(i.pressuresource)
    )
    print(
        "radiation: "
        + str(i.radiation)
        + ", radiation source: "
        + str(i.radiationsource)
    )
    if len(i.music) > 0:
