        ...
        for i in range(num_images):
            ...
            for j in range(heights[i]):
                ...
                while p < widths[i]:
                    ...
                    if is_color:
                        data[
                            (j * widths[i] + p) * 2 : (j * widths[i] + p + run_length)
                            * 2
...
def stitch_to_atlas(images):
    if not all(_.mode == images[0].mode for _ in images):
        raise ValueError(
            "All images must be in the same color mode to stich to an atlas"
        )

    sorted_images = sorted(images, key=lambda i: -max(i.width, i.height))
    num_actual_pixels = sum(img.width * img.height for img in sorted_images)

    # calculate packing, find best max_width
    best_efficiency = 0
    best_max_width = None
    best_final_y = None
    best_packing = None
    for max_width in (64, 128, 256, 512, 1024, 2048):
        packing = []
        x = 0
        y = 0
        max_x = 0
        ...
        for img in sorted_images:
            ...
            x += img.width
            max_x = max(max_x, x)
            current_row_max_y = max(current_row_max_y, img.height)
        final_y = y + current_row_max_y
        efficiency = num_actual_pixels / (max_x * final_y)
        if efficiency > best_efficiency:
            best_efficiency = efficiency
            best_max_width = max_x
            best_final_y = final_y
            best_packing = packing

    # print(f"best dimensions {best_max_width}x{best_final_y}")

    # set up storage
    newimage = Image.new(images[0].mode, size=(best_max_width, best_final_y))
    if newimage.mode == "P":
    ...
    # do the packing
    for img, xy in zip(sorted_images, best_packing):
        newimage.paste(img, (xy[0], xy[1], xy[0] + img.width, xy[1] + img.height))

    # print some stats
    total_pixels = newimage.width * newimage.height
    print(
        f"Packed {num_actual_pixels:,}px into {total_pixels:,}px, efficiency ratio {num_actual_pixels/total_pixels*100:f}%"
    )
